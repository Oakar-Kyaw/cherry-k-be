...
exports.listAllExpenses = async (req, res) => {
  ...
  try {
    ...
    let result = await Expense.find(query)
...
exports.expenseBankCashFilter = async (req, res) => {
  ...
  try {
    ...
    let bankResult = await Expense.find(query).populate(
      "relatedBankAccount relatedCashAccount relatedAccounting relatedBranch"
    );

    const { relatedBankAccount, ...query2 } = query;
    query2.relatedCashAccount = { $exists: true };
    let cashResult = await Expense.find(query2).populate(
      "relatedBankAccount relatedCashAccount relatedAccounting relatedBranch"
    );
    const CashNames = cashResult.reduce(
      (result, { relatedCashAccount, finalAmount }) => {
    ...
    const CashTotal = cashResult.reduce(
      (total, sale) => total + (sale.finalAmount || 0),
      0
    );
    response.data = {
      ...response.data,
    ...
    const BankNames = bankResult.reduce(
      (result, { relatedBankAccount, finalAmount }) => {
...
exports.getCode = async (req, res) => {
  ...
  try {
    ...
    const latestDocument = await Expense.find({}, { seq: 1 })
      .sort({ _id: -1 })
      .limit(1)
      .exec();
    if (latestDocument.length === 0)
      data = { ...data, seq: 1, code: "EPC-" + "-1" }; // if seq is undefined set initial patientID and seq
    if (latestDocument.length > 0) {
...
exports.createExpense = async (req, res, next) => {
  try {
    ...
    if (latestExpense && latestExpense.code) {
      const latestSequence = parseInt(latestExpense.code.split("-")[2], 10);
    ...
    const populatedResult = await Expense.find({ _id: result._id })
      .populate("relatedBranch")
      .populate("relatedAccounting")
      .populate("relatedBankAccount")
      .populate("relatedCashAccount");
    ...
    if (req.body.relatedCredit) {
      ...
      const secondTransaction = {
        ...
        treatmentFlag: false,
        relatedTransaction: fTransResult._id,
        relatedAccounting: newBody.relatedBankAccount
          ? newBody.relatedBankAccount
          : newBody.relatedCashAccount,
        relatedExpense: result._id,
...
exports.getwithExactDate = async (req, res) => {
  try {
    let { date } = req.query;
    let result = await Expense.find({ date: date })
      .populate("relatedBranch")
      .populate("relatedAccounting")
      .populate("relatedBankAccount")
      .populate("relatedCashAccount");
...
exports.expenseFilter = async (req, res) => {
  ...
  try {
    ...
    const bankResult = await Expense.find(query)
      .populate(
        "relatedBankAccount relatedAccounting relatedCredit relatedCashAccount relatedBranch"
      )
      .populate("createdBy", "givenName");
    const { relatedBankAccount, ...query2 } = query;
    query2.relatedCashAccount = { $exists: true };
    console.log(query2);
    const cashResult = await Expense.find(query2)
      .populate(
        "relatedBankAccount relatedAccounting relatedCredit relatedCashAccount relatedBranch"
      )
      .populate("createdBy", "givenName");
    ...
    const BankTotal = bankResult.reduce((total, sale) => {
      let current = currencyList.filter(
        (currency) => currency.code === sale.finalCurrency
      )[0].exchangeRate;
