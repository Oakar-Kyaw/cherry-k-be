...
exports.listMultiTreatmentSelections = async (req, res) => {
  ...
  try {
    ...
    let result = await TreatmentSelection.find(query)
      .populate(
        "createdBy relatedBranch relatedTreatmentList relatedAppointments relatedPatient finishedAppointments remainingAppointments relatedTransaction"
      )
      .populate({
...
exports.createMultiTreatmentSelection = async (req, res, next) => {
  ...
  try {
    ...
    let checkDuplicate = await checkDuplicateVoucher({
      tsType: tsType,
    ...
    for (const i of parsedPackage) {
      ...
      //related account for each treatments
      //search treatmentPackage if include treatment package
      getPackageAccountingAccount = await TreatmentPackages.findOne({
        _id: i.item_id,
      }).populate({
        path: "relatedTreatment",
      });
      if (getPackageAccountingAccount.relatedTreatment) {
        ...
        for (const treatment of getPackageAccountingAccount.relatedTreatment) {
          console.log(
            "length",
            getPackageAccountingAccount.relatedTreatment.length,
            i.price,
            data
          );
          let treatmentData = {
          ...
          treatmentData.totalAmount =
            i.price / getPackageAccountingAccount.relatedTreatment.length;
      ...
      if (getPackageAccountingAccount.relatedAccount) {
        ...
        const amtUpdate = await Accounting.findOneAndUpdate(
          { _id: getPackageAccountingAccount.relatedAccount },
          { $inc: { amount: req.body.totalPaidAmount } }
        );
      }
    ...
    if (treatmentVoucherResult) {
      var populatedTV = await TreatmentVoucher.find({
        ...
        .populate("relatedDiscount multiTreatment.item_id")
        .populate({
...
exports.TopTenFilter = async (req, res) => {
  try {
    ...
    const treatmentNameMap = TreatmentResult.reduce(
      (result, { relatedTreatment, multiTreatment, purchaseType }) => {
        if (relatedTreatment) {
