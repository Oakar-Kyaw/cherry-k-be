...
exports.confirmIssueToHo = async (req, res) => {
  ...
  try {
    ...
    const queryTransferToHoRequest = await TransferToHoRequest.findOne({
      _id: id,
    });
    //check if medicineItems exists
    if (
      queryTransferToHoRequest.medicineItems &&
      queryTransferToHoRequest.medicineItems.length != 0
    ) {
      // if there are , loop
    ...
    if (
      queryTransferToHoRequest.procedureItems &&
      queryTransferToHoRequest.procedureItems.length != 0
    ) {
      // if there are , loop
    ...
    if (
      queryTransferToHoRequest.accessoryItems &&
      queryTransferToHoRequest.accessoryItems.length != 0
    ) {
      // if there are , loop
    ...
    if (
      queryTransferToHoRequest.generalItems &&
      queryTransferToHoRequest.generalItems.length != 0
    ) {
      // if there are , loop
