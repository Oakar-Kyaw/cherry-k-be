...
exports.listAllProcedureHistorys = async (req, res) => {
  ...
  try {
    ...
    let result = await ProcedureHistory.find(query).populate(
      "medicineItems.item_id customTreatmentPackages.item_id pHistory relatedAppointment relatedTreatmentSelection"
    );
...
exports.getProcedureHistory = async (req, res) => {
  const result = await ProcedureHistory.find({
    _id: req.params.id,
    isDeleted: false,
  }).populate(
    "medicineItems.item_id customTreatmentPackages.item_id pHistory relatedAppointment relatedTreatmentSelection"
...
exports.getRelatedProcedureHistory = async (req, res) => {
  let { relatedAppointment, relatedTreatmentSelection, relatedBranch } =
    req.query;
  ...
  try {
    const result = await ProcedureHistory.find(query).populate(
      "medicineItems.item_id customTreatmentPackages.item_id pHistory relatedAppointment relatedTreatmentSelection"
    );
...
exports.createProcedureHistory = async (req, res, next) => {
  ...
  try {
    ...
    const populate = await procedureHistory
      .find({ _id: result._id })
      .populate(
        "medicineItems.item_id customTreatmentPackages.item_id pHistory relatedAppointment relatedTreatmentSelection"
      );
...
exports.updateProcedureHistory = async (req, res, next) => {
  ...
  try {
    ...
    const result = await procedureHistory
      .findOneAndUpdate({ _id: req.body._id }, data, { new: true })
      .populate(
        "medicineItems.item_id customTreatmentPackages.item_id pHistory relatedAppointment relatedTreatmentSelection"
      );
