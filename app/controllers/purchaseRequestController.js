...
exports.listAllPurchaseRequests = async (req, res) => {
    ...
    try {
        ...
        let result = await PurchaseRequest.find(query).populate('medicineItems.item_id').populate('procedureItems.item_id').populate('relatedBranch').populate('accessoryItems.item_id').populate('generalItems.item_id')
...
exports.createPurchaseRequest = async (req, res, next) => {
    let data = req.body
    let { relatedBranch, medicineItems, procedureItems, accessoryItems, generalItems } = data
    try {
        ...
        if(medicineItems.length != 0){
            loop(medicineItems.length, async (index)=>{
                ...
                let changeMedicine = { purchasePrice: medicineItems[index].purchasePrice }
                let medicineItemQuery = await MedicineItems.findByIdAndUpdate(id,
                    changeMedicine)
            })
        }
        if(procedureItems.length != 0){
            loop(procedureItems.length, async (index)=>{
                ...
                let changeProcedureItems = { purchasePrice: procedureItems[index].purchasePrice }
                let procedureItemsQuery = await ProcedureItems.findByIdAndUpdate(id,
        ...
        if(accessoryItems.length != 0){
            loop(accessoryItems.length, async (index)=>{
                ...
                let changeAccessoryItems = { purchasePrice: accessoryItems[index].purchasePrice }
                let changeAccessoryItemsQuery = await AccessoryItems.findByIdAndUpdate(id,
        ...
        if(generalItems.length != 0){
            loop(generalItems.length, async (index)=>{
                ...
                let changeGeneralItems = { purchasePrice: generalItems[index].purchasePrice }
                let medicineItemQuery = await GeneralItems.findByIdAndUpdate(id,
                    changeGeneralItems)
            })
        }
        res.status(200).send({
...
exports.updatePurchaseRequest = async (req, res, next) => {
    try {
        const result = await PurchaseRequest.findOneAndUpdate(
            ...
            req.body,
            { new: true },
        ).populate('medicineItems.item_id').populate('procedureItems.item_id').populate('accessoryItems.item_id').populate('generalItems.item_id')
        return res.status(200).send({ success: true, data: result });
    } catch (error) {
        return res.status(500).send({ "error": true, "message": error.message })
    }
