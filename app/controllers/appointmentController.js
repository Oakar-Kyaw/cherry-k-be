...
exports.updateAppointment = async (req, res, next) => {
  try {
    ...
    const treatmentSelect = await TreatmentSelection.findById(treatmentSelectId)
    //if status of all appoointment is true, 'ongoing' flag wil be true
    let filter = treatmentSelect.relatedAppointments.filter(appointments => appointments.status == false)
    if(filter.length == 0) await TreatmentSelection.findByIdAndUpdate(treatmentSelectId, {selectionStatus:"Done"})
    return res.status(200).send({ success: true, data: result, treatmentSelect: treatmentSelect });
  } catch (error) {
    return res.status(500).send({ "error": true, "message": error.message })
  }
};
