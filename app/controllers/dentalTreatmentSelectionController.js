...
exports.listMultiDentalTreatmentSelections = async (req, res) => {
  ...
  try {
    ...
    let result = await dentalTreatmentSelection
      .find(query)
      .populate(
        "createdBy relatedBranch relatedDentalTreatmentList relatedAppointments relatedPatient finishedAppointments remainingAppointments relatedTransaction"
      )
      .populate({
...
exports.getDentalTreatmentSelection = async (req, res) => {
  ...
  const result = await dentalTreatmentSelection
    .find(query)
    .populate(
      "createdBy relatedAppointments remainingAppointments relatedTransaction relatedPatient relatedDentalTreatmentList"
    )
    .populate({
      path: "relatedDentalTreatment",
      populate: [
...
exports.createDentalTreatmentSelectionCode = async (req, res) => {
  ...
  try {
    //prepare TS-ID
    const latestDocument = await dentalTreatmentSelection
      .find({}, { seq: 1 })
      .sort({ _id: -1 })
      .limit(1)
      .exec();
    if (latestDocument[0].seq === undefined)
      data = { ...data, seq: 1, code: "TS-1" }; // if seq is undefined set initial patientID and seq
...
exports.createMultiDentalTreatmentSelection = async (req, res, next) => {
  ...
  try {
    ...
    for (const i of parsedMulti) {
      ...
      const getAccountingAcccount = await dentalTreatment.findOne({
        _id: i.item_id,
      });
      if (getAccountingAcccount.relatedAccount) {
        ...
        const amtUpdate = await Accounting.findOneAndUpdate(
          { _id: getAccountingAcccount.relatedAccount },
    ...
    if (tvcCreate === true) {
      ...
      if (dentalTreatmentPackage && dentalTreatmentPackage.length != 0) {
        ...
        dentalTreatmentPackage.map((dentalTreatment) => {
          dataTVCDentalTreatmentPackageArray.push({
            item_id: dentalTreatment.item_id,
            qty: dentalTreatment.qty,
          });
          treatmentPackageArray.push(treatment.item_id);
        });
        dataTVC["relatedDentalTreatmentPackage"] =
          dataTVCDentalTreatmentPackageArray;
      }
    ...
    if (treatmentVoucherResult) {
      var populatedTV = await TreatmentVoucher.find({
        ...
        .populate("relatedDiscount multiDentalTreatment.item_id")
        .populate({
          path: "relatedDentalTreatmentPackage",
    ...
    var updatePatient = await Patient.findOneAndUpdate(
      { _id: relatedPatient },
      {
        $addToSet: { relatedDentalTreatmentSelection: TSArray },
        $push: {
          relatedDentalTreatmentPackage: { $each: treatmentPackageArray },
        },
        $inc: {
      ...
      if (totalSelectionLength.length != 0) {
        ...
        for (let i = 0; i < totalSelectionLength.length; i++) {
          ...
          let updateTreatmentSelectionRefund =
            await dentalTreatmentSelection.findByIdAndUpdate(
...
exports.createDentalTreatmentSelection = async (req, res, next) => {
  ...
  try {
    ...
    if (req.body.paymentMethod === "Advance") {
      const treatmentResult = await dentalTreatment.find({
        _id: req.body.relatedDentalTreatment,
      });
      let advanceAmount = req.body.totalAmount - req.body.paidAmount;

      if (
        req.body.deferAmount > 0 &&
    ...
    const freqUpdate = await Patient.findOneAndUpdate(
      { _id: req.body.relatedPatient },
      {
        $inc: {
          ...
          totalAppointments: req.body.dentalTreatmentTimes,
          unfinishedAppointments: req.body.dentalTreatmentTimes,
        },
      },
      { new: true }
    );
    ...
    //Freq Update end
    const populatedResult = await dentalTreatmentSelection
      .find({ _id: result._id })
      .populate(
        "createdBy relatedAppointments remainingAppointments relatedTransaction relatedPatient relatedDentalTreatmentList"
      )
...
exports.updateDentalTreatmentSelection = async (req, res, next) => {
  try {
    ...
    const result = await dentalTreatmentSelection
      .findOneAndUpdate({ _id: req.body.id }, data, { new: true })
...
exports.dentalTreatmentPayment = async (req, res, next) => {
  ...
  try {
    let { paidAmount } = data;
    const treatmentSelectionQuery = await dentalTreatmentSelection
      .find({ _id: req.body.id, isDeleted: false })
      .populate("relatedDentalTreatment")
    ...
    if (result.paymentMethod === "Credit") {
    ...
    } else if (result.paymentMethod === "Cash Down") {
      //byAppointment
      ...
      var rpRecordPopulated = await Repay.find({
        _id: repayRecord._id,
...
exports.getRelatedDentalTreatmentSelections = async (req, res) => {
  try {
    ...
    return res.status(500).send({
      error: true,
      message:
        "An Error Occured While Fetching Related Dental Treatment Selections",
    });
  }
};
...
exports.TopTenFilter = async (req, res) => {
  try {
    ...
    const TreatmentResult = await dentalTreatmentSelection
      .find(query)
      .populate("relatedDentalTreatment")
    ...
    const treatmentNameMap = TreatmentResult.reduce(
      (
        result,
        { relatedDentalTreatment, multiDentalTreatment, purchaseType }
      ) => {
        if (relatedDentalTreatment) {
          const { name, dentalTreatmentName, sellingPrice } =
