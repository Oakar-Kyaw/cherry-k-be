...
exports.listAllTreatments = async (req, res) => {
  ...
  try {
    ...
    let result = await Treatment.find(query).populate('relatedDoctor').populate('relatedTherapist').populate('relatedPatient').populate('machine.item_id').populate('procedureAccessory.item_id').populate('medicineLists.item_id').populate('procedureMedicine.item_id').populate('treatmentName').populate({
...
exports.getTreatment = async (req, res) => {
  const result = await Treatment.find({ _id: req.params.id, isDeleted: false }).populate('relatedDoctor').populate('relatedTherapist').populate('relatedPatient').populate('machine.item_id').populate('procedureAccessory.item_id').populate('medicineLists.item_id').populate('procedureMedicine.item_id').populate('treatmentName').populate({
    path: "general.item_id",
    model: "GeneralItems",
    populate: {
      path: "name",
...
exports.updateTreatment = async (req, res, next) => {
  try {
    const result = await Treatment.findOneAndUpdate(
      ...
      req.body,
      { new: true },
    ).populate('relatedDoctor').populate('relatedTherapist').populate('relatedPatient').populate('').populate('machine.item_id').populate('procedureAccessory.item_id').populate('medicineLists.item_id').populate('procedureMedicine.item_id').populate('treatmentName').populate({
      path: "general.item_id",
      model: "GeneralItems",
      populate: {
        path: "name",
...
exports.searchTreatments = async (req, res, next) => {
  try {
    const result = await Treatment.find({ $text: { $search: req.body.search } }).populate('relatedDoctor').populate('relatedTherapist').populate('relatedPatient').populate('').populate('machine.item_id').populate('procedureAccessory.item_id').populate('medicineLists.item_id').populate('procedureMedicine.item_id').populate('treatmentName').populate('general.item_id')
...
exports.getRelatedTreatmentByTreatmentListID = async (req, res) => {
  try {
    const result = await Treatment.find({ treatmentName: req.params.id, isDeleted: false })
...
exports.getDataToExportExcel = async (req, res, next) => {
  try {
    let medicineLists = []
    let queryDataList = await Treatment.find({ treatmentName: req.params.id, isDeleted: false })
      .populate({
        path: 'treatmentName',
